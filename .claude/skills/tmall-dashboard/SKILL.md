...
3. **价格方向别搞反。** 价差 = 官方指导价 − 发现价格，所以低价是**正**价差。
   ...
   「异常价格」标识但**照样计入**低价统计。「涉及店铺」只数**有低价链接**的店铺
   （和源表「看板」页一致），不是在架店铺总数。

源表的两个坑：子表叫「排查登记表」不是「排查记录」；「发现价格」那一列实际叫
**「违规售价」**。源表里还有老款 `PLAUD NOTE` / `PLAUD NotePin`，而人工看板只统计
`Note Pro` + `NotePin S` —— 所以本页不筛型号时是 112 条，看板上写的是 93 条，
这不是算错了。

列定义（标签、宽度、单元格渲染）集中在页面的 `columns()` 里，colgroup、表头、
单元格和表宽都从它生成，**改列宽只改那一处**。固定列的 left/right 由
`syncStickyOffsets()` 在每次渲染表头后按实际列宽写进 CSS 变量 —— 写死像素值
只在「表宽 = 各列之和」时才成立，容器一旦比表格宽，列会被等比拉伸、固定列立刻错位。

**配色按产品给的参考稿走**（对方看过两版单色方案后明确要这个）：亮蓝主色，风险四档
沿「蓝（轻度）→ 橙（中度）→ 深橙（高风险）→ 红（严重）」渐进，标签一律浅底深字；
KPI 卡左侧是六色图标块（`--t-*`），发现价格列有一层淡黄底把主角挑出来。
改配色前先确认对方要的是哪一版 —— 这里换过两轮，别凭审美自己推翻。

图表里的「非低价」是中性灰（`--c-none`），它占柱子七八成，调太浅整根柱子就看着是空的；
非当期柱子的 `.dim` 也别低于 0.6，趋势图的重点恰恰是历史几期。

条榜（产品分布 / 店铺 TOP / 发货地）的三列列宽由**外层** `.hbars` 的 grid 决定，
每行 `display: contents`。别改回「每行自己是一个 grid」—— 那样列宽各算各的，
名称一长那行的条就右移，十行下来条的左缘参差不齐。
