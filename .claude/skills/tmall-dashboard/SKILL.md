...
### 往源表里写数据（`npm run import:search`）

排查记录那张表**可以写**（应用的 sheets 权限是读写，不是只读）。但写之前记住：

**飞书会把请求区间里的空行一起返回。** 问 `A1:Q20000` 就还你 20000 行，拿
`values.length` 当「最后一行」会把数据写到第 20001 行去。读写位置前先截掉末尾空行
（`readRange()` 里做了）。不带区间地读整张子表则只返回已用区域，两种行为不一样。

**搜索页展示的是最低 SKU 价，不是整机价。** 同一个链接挂「主机」「主机+会员」
「保护套」几个 SKU，搜索页只显示最便宜那个。表里那几行 ¥158 / ¥189 的「严重低价」
全是这么来的，真实整机价是 ¥1508 / ¥1472。所以导入脚本默认**不写**，必须显式
`--write`，并且把 ≥40% 的行单独标出来提示人工复核。看到「严重低价」暴涨先怀疑这个，
别急着报警。

**低价幅度那一列是百分比格式的单元格。** 接口读回来的是小数（-2.174 显示成 -217.4%），
往里写数字时也要写小数。曾经有 4 行存着 -0.02174，显示 -2.17%，比真值小 100 倍。

**批次之间的数字只有在抓取口径一致时才可比。** 9-14 那批 112 条、9-22 那批 64 条，
差的是抓取范围（淘宝只存了 1 页 SEM 落地页），不是市场变化。趋势图会如实画出
「在架链接 -43%」，那是假的。换口径了就要说清楚，别让它混进环比。

解析器的坑写在 `lib/lowprice/import/` 的注释里，核心两条：淘宝把店铺 `<a>` 嵌在商品
`<a>` 里（HTML 不允许 a 嵌套，浏览器会提前闭合外层 a，店名变成兄弟节点），且同一商品
会渲染多张卡、价格和店名分散在不同卡上，只能**按 id 合并字段**；京东每张卡自带
`data-item` JSON，直接读它，但它没有发货地，月销只有累计评价（「1万+」要先换算成
10000，否则解析器读成 1）。
