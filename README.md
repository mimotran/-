...
components/
  ...
  lowprice/              低价链接监测（链接明细页，和天猫看板各走各的数据源）
    rules.js             口径唯一实现：低价判断 / 幅度 / 风险等级 / 筛选 / 排序 / 聚合
    rules.d.ts           上面那份纯 JS 的类型声明
    parse.ts             「排查记录」→ LinkRecord（按表头名找列 + 校验和告警）
    feishu.ts            读取「排查记录」子表
    snapshot.ts          快照落盘
    types.ts             数据模型
scripts/
  sync.ts                本地同步与连通性自检
  ...
  sync-links.ts          拉「排查记录」+ 表头对齐报告 + 最新一天的五个指标
  build-links.ts         注入口径与数据 → dist/links.html
  check-links.ts         链接明细页对账（页面口径 vs lib 口径）
  env.ts                 .env.local 加载
```
...
另一条独立的线：数据源是「PLAUD 低价链接监测」工作簿里的 **「排查记录」** 子表，
产物是 `dist/links.html`（预览源文件 `preview/links.html`），和天猫看板互不影响。

```bash
npm run sync:links     # 拉一次并自检：子表清单、表头对齐、最新一天的五个指标
npm run build:links    # 注入口径 + 数据 → dist/links.html
npm run check:links    # 对账：页面里的口径必须和 lib 里的完全一致
```

### 口径

```
价差(元)  = 官方指导价 − 发现价格          指导价 ¥1,399、发现价 ¥1,310 → +¥89
低价幅度  = 价差 ÷ 官方指导价                                          → 6.36%
低价      = 发现价格 < 官方指导价（即价差 > 0）
```

风险分档（边界都算进重的一档）：轻度 `0 < r < 5%`、中度 `5% ≤ r < 10%`、
高风险 `10% ≤ r < 20%`、严重 `r ≥ 20%`；`r ≥ 40%` 另打「⚠ 异常价格」标识
（疑似配件 / 定金 / 优惠券 / 非完整商品 / 抓取错误），**仍然计入低价统计**，不自动剔除。
缺官方指导价或发现价格的行算「缺口径」：计入「在架链接」，不计入低价统计。

店铺去重优先用**卖家 ID/UID**，为空时才退回「平台 + 卖家昵称」——
店铺名会改、会撞名，单用名称去重会把两家店并成一家。

### 只有一份口径

`lib/lowprice/rules.js` 是**唯一实现**，`npm run build:links` 把它原样注入页面，
Node 侧自检和浏览器里跑的是同一段代码。`npm run check:links` 会验证这件事
（逐字节比对 + 逐行、逐筛选组合、逐排序键比结果），CI 里也跑，对不上就发布失败。

所以改口径只改 `lib/lowprice/rules.js`，然后：

```bash
npm run build:links -- --write-preview   # 顺带把 rules 块写回 preview/links.html
```

### 表头对不上

按**表头名**找列，不按 A/B/C 列号（源表插一列不会让页面失效）。
认不出的列在 `lib/lowprice/parse.ts` 的 `LINK_ALIASES` 里加一个别名即可，
`npm run sync:links` 会逐字段打印「字段 → 命中的列名」。

源表自己算过的「价差」「低价幅度」会和现算值对一遍，差得离谱就告警（页面以原始价格为准）。

### 页面

- 顶部筛选：排查日期（默认最新一天，支持区间）、平台、产品型号、版本、店铺类型、
  是否低价、低价程度、发货地、关键词（店铺名 / 卖家 ID / 商品链接 / 备注）
- 五个指标：在架链接 / 低价链接 / 低价占比 / 涉及店铺 / 严重低价，全部按当前筛选结果实时算
- 明细表：默认按风险优先排序（严重 → 高风险 → 中度 → 轻度 → 非低价，同级按幅度降序），
  可按发现价格 / 价差 / 低价幅度 / 月销 / 排查日期排序，分页 20 / 50 / 100
- 左侧固定平台、店铺名称、产品型号，右侧固定风险等级与操作；价格四件套紧跟固定列，
  保证不横向滚动也能看到「便宜了多少」
- 商品链接不展示裸 URL，只给「查看商品」「复制链接」；店铺给「查看店铺」
- 支持从看板跳转带参数：`?date=2026-09-14&platform=淘宝&product=Note Pro&version=海外版&lowPrice=true`
  （型号少写前缀也认；实在匹配不上会退回「全部」并在横幅里说明，而不是筛出一张空表）

**没取到数据时页面是空态**，不会拿演示数据占位 —— 和看板同一条规矩：旧但真 > 新但编。
