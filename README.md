...
另一条独立的线：数据源是「PLAUD 低价链接监测」工作簿里的**逐条巡检记录**（子表实际名叫
「排查登记表」，sheet id `1WDztC`），产物是 `dist/links.html`（预览源文件
`preview/links.html`），和天猫看板互不影响。

同一个工作簿里的「看板」页是**人工汇总的展示层**，代码里从不读它 —— 读了就等于把二手数字
当事实表用，源表一改两边就对不上。
...
「涉及店铺」= **有低价链接的**店铺去重数，不是在架店铺总数（KPI 小字里给全量）。
口径取自源表「看板」页：运营看这个数是为了知道要找多少家店谈。
...
趋势图里「链接数」和「低价占比」量纲不同。两条真值轴怎么对齐是任意的，会凭空造出
并不存在的相关性，所以这里不把它们塞进同一根轴：**数量走堆叠柱**（左轴，还顺带
把风险构成画出来了），**占比走虚线 + 空心点**（右轴，上限按实际最大值上浮一档、
永远从 0 起），视觉上明确分层。KPI 卡里的迷你折线只表示走势，不画轴也不标值。

### 和源表「看板」页对过账

2026-09-14 那天，把明细页筛成看板页的口径（只看 `PLAUD Note Pro` + `PLAUD NotePin S`
两个在售型号，老款 `PLAUD NOTE` / `PLAUD NotePin` 共 19 条不在内），得到：

| | 在架链接 | 低价链接 | 低价占比 | 涉及店铺 |
|---|---|---|---|---|
| 本页现算 | 93 | 33 | 35.5% | 23 |
| 源表「看板」页 | 93 | 33 | 35.5% | 23 |

型号 × 平台 × 版本 的 8 个分组（12/16、6/12、5/17、0/12、4/7、5/8、1/18、0/3）也逐格相同。
**不带型号筛选时是 112 / 37 / 33.0% / 26** —— 多出来的是老款，不是算错了。

### 表头对不上

按**表头名**找列，不按 A/B/C 列号（源表插一列不会让页面失效）。
已经踩过一次：需求文档里的「发现价格」，源表那一列实际叫 **「违规售价」**。
...
- 顶部筛选：排查批次（源表每周一批，日期即批次）、自定义区间、平台、产品型号、版本、
  店铺类型、是否低价、低价程度、发货地、关键词（店铺名 / 卖家 ID / 商品链接 / 备注）；
  快捷按钮「最新一期 / 近 4 期 / 近 8 期 / 全部批次」
- **跨批次时的去重开关**：同一条链接每期都会被抓一次，勾上「同一链接只看最新一次」后，
  KPI、图表、表格用的是同一份去重结果，不会出现「指标去了重、表格没去重」
- 六个指标：在架链接 / 低价链接 / 低价占比 / 涉及店铺 / 严重低价 / 低价均差，
  全部按当前筛选结果实时算，每格带**环比**（按批次比，不按自然天）和迷你走势线
- **趋势图**：一期一根堆叠柱（按风险分档着色），叠一条低价占比虚线；点柱子直接切到那一期。
  它只受平台 / 型号等条件影响，**不受日期筛选影响** —— 否则单期视图下图上只剩一个点
- 风险分级环形图（只画低价四档，中心数字＝低价链接数，非低价单列在图例里）、
  产品 / 平台 / 版本分布、高风险店铺 TOP 10、发货地分布
- 明细表：默认按风险优先排序（严重 → 高风险 → 中度 → 轻度 → 非低价，同级按幅度降序），
  可按发现价格 / 价差 / 低价幅度 / 月销 / 排查日期排序，分页 20 / 50 / 100
- 「排查日期」列只在跨批次时出现：单期视图里整列都是同一个日期，纯占位，
  而它占掉的宽度正好够备注多显示半行
- 左侧固定平台、店铺名称、产品型号，右侧固定风险等级与操作；价格四件套 + 备注紧跟固定列，
  保证不横向滚动也能看到「便宜了多少」和「为什么可疑」（源表备注写的是判断依据，
  排在最后会被右侧固定列整列盖掉）
- 商品链接不展示裸 URL，只给「查看商品」「复制链接」；店铺给「查看店铺」
- 支持从看板跳转带参数：`?date=2026-09-14&platform=淘宝&product=Note Pro&version=海外版&lowPrice=true`；
  另有 `?periods=4`（最近 4 期）、`?date=all`（全部批次）、`?dedupe=1`（按链接去重）
