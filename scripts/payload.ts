 ...
 * 看板数据载荷：`buildPayload()`
 *
 * 导出 JSON（npm run export）和生成静态站（npm run build:static）都要这一份，
 * 抽出来共用 —— 两处各拼一遍的话，加了字段只改一处，静态站就会悄悄少一块数据。
 */
...
export async function buildPayload() {
  ...
  return { payload, source: snapshot.source, latestDate: view.latestDate };
}
